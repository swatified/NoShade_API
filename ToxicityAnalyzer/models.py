...
class ToxicityAnalyzer:
    ...
    def get_sentiment(self, text):
        try:
            ...
            if not self.sentiment_analyzer:
                # Default to neutral if analyzer isn't available
                return [0, 1, 0]
            ...
            # Default to neutral if unknown label
    ...
    def analyze(self, text):
        try:
            ...
            max_sentiment_idx = sentiment_features.index(
                max(sentiment_features))
...
class Content(models.Model):
    ...
    def __str__(self):
        return f"Content {self.external_id}"

class Comment(models.Model):
    content = models.ForeignKey(Content, on_delete=models.CASCADE, related_name='comments')
    text = models.TextField()
    sentiment = models.CharField(max_length=50)
    toxic_labels = models.TextField(blank=True)
    is_toxic = models.BooleanField(default=False)
    analyzed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name='analyzed_comments'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"Comment {self.id} on {self.content}"
